...
export default function ManageUsersPage() {
  ...
  const handleAddUser = () => {
    if (newUserName.trim() && newUserUsername.trim() && newUserPassword.trim()) {
      addUser(newUserName.trim(), newUserUsername.trim(), newUserPassword, newUserRole);
      setNewUserName('');
      setNewUserUsername('');
      setNewUserPassword('');
      setNewUserRole('user');
      ...
      <Card className="shadow-md">
        ...
        <CardContent className="space-y-4">
            ...
            <div className="flex justify-end pt-2">
                <Button onClick={handleAddUser} disabled={!newUserName.trim() || !newUserUsername.trim() || !newUserPassword.trim()} className="w-full md:w-auto">
            ...
            {users.map(user => (
              <Card key={user.id} className="shadow-lg hover:shadow-xl transition-shadow duration-300">
                <CardHeader>
                  ...
                  <CardDescription>ID: {user.id} | Username: {user.username}</CardDescription>
                </CardHeader>
                <CardContent>
